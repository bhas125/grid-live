...
async function pullCad(): Promise<CrimeIncident[]> {
  ...
  const jobs = rows.slice(0, 24).map(async (a) => {
    const loc = String(a.Location ?? "").trim();
    if (!loc) return;
    const city = String(a.CityName ?? "Nashville").trim() || "Nashville";
    const xy = await geocode(loc, /nashville|east|north|south|west|bellevue|madison|antioch|donelson|hermitage|inglewood/i.test(city) ? "Nashville" : city);
    if (!xy) return;
    const id = `CAD-${a.ObjectId ?? loc}`;
    out.push({
      id,
      date: a.CallReceivedTime ? ymd(a.CallReceivedTime) : ymd(Date.now()),
      city: city === "EAST" || city === "NORTH" || city === "SOUTH" || city === "WEST" ? "Nashville" : city,
      county: "Davidson",
      address: loc,
      lat: xy.lat,
      lon: xy.lon,
